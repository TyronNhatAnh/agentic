...
# Tools that require a Slack-button confirm. Bare names — `_needs_confirm` strips
# the `mcp__<server>__` prefix. Membership here only works while the tool is in no
# `allowed_tools` list, which the SDK would short-circuit past the callback.
# Deliberately absent: `github_approve_pr` (the review flow auto-LGTMs, approval is
# reversible, merge is still gated) and `db_query_prod` (one prod turn fans out to
# many queries; writes are impossible and every call is audit-logged server-side).
