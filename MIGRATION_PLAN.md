...
- **2026-05-30** — Phase 5 — Slack-pass fix: thêm MCP tool **`list_services`**. Vấn đề: "review PR #1077 ở order" → brain **đoán nhầm slug** `gogox/order-service` (sai org + sai tên; đúng là `gogovan/ggx-kr-order-service`). Registry CÓ alias "order" (`resolve_service('order')` ăn) nhưng **brain không có tool nào tra registry** → phải đoán. Fix: `list_services` (đọc `store.list_services`, trả name/github_repo/repo_path/aliases). Guidance đặt trong **tool description** ("call this... never guess the owner/repo slug") — KHÔNG nhồi prose vào brain.md (tránh overfit; domain rule "Service names... đừng đoán" đã có sẵn). Read-only local, không retry. File: [mcp_tools.py](src/agentic/sdk/mcp_tools.py). 34 tools, 43 pass.
